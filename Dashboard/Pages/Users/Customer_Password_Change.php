  ...
  if(isset($_POST['btnsubmit']))
   {
	 ...
	 if($npasswd==$rpasswd)
	 {
		 $selectQ="select * from tbl_customer_reg where customer_id='".$cid."'";
		 $row=mysqli_query($con,$selectQ);
	     $data=mysqli_fetch_assoc($row);
		 if($data['customer_password']==$cpasswd)
		 {
			 $updateQ="update tbl_customer_reg set customer_password='".$npasswd."' where customer_id='".$cid."'";
			 mysqli_query($con,$updateQ);
			 header("Location:Customer_profile.php");
		 }
		 else echo "Current Password Does not match...";
     }
	 else echo "New Password and Retype Password do not match...";
   }
