<?php
	...
	if(isset($_POST['btnsubmit']))
	{
		...
		$selE="select * from  tbl_station_reg where station_username='".strtolower($_POST['name'])."'";
		$rowE=mysqli_query($con,$selE);
		$numE=mysqli_num_rows($rowE);
		
		
		$selC="select * from  tbl_customer_reg where customer_username='".strtolower($_POST['name'])."'";
		$rowC=mysqli_query($con,$selC);
		$numC=mysqli_num_rows($rowC);
		
		if($numE>0)
		{
			$dataEmail=mysqli_fetch_assoc($rowE);
			$Email=$dataEmail['station_email'];
			$_SESSION['Fid']=2;
			$_SESSION['Fuser']=$dataEmail['station_reg_id'];
		}
		else if($numC>0)
		{
			$dataEmail=mysqli_fetch_assoc($rowC);
			$Email=$dataEmail['customer_email'];
			$_SESSION['Fid']=3;
			$_SESSION['Fuser']=$dataEmail['customer_id'];
			echo $_SESSION['Fuser']."hai";
		}
		else{
      ...
      <div class="container">
        <div class="row no-gutters slider-text align-items-end justify-content-center">
          <div class="col-md-5 ftco-animate pb-5 text-center" style="margin-top: 100px;">
            ...
            <p class="breadcrumbs"><span class="mr-2"><a href="index.php">Home <i class="ion-ios-arrow-forward"></i></a></span> <span>Forgot Passsword <i class="ion-ios-arrow-forward"></i></span></p>
			<form action="#" method="post" class="contact-form">
			  ...
			  <div class="form-group">
                <input type="submit" name="btnsubmit" value="Submit" class="btn btn-primary py-3 px-5">
              </div>
            </form>
		  </div>
        </div>
